...
namespace cura
{
    ...
    paths(paths),
    skin_position_type(skin_position_type),
...
{
    // Settings & configs:
    const auto pack_by_inset = !storage.getSettingBoolean("optimize_wall_printing_order");
    const auto inset_direction = storage.getSettingsAsInsetDirection("inset_direction");
    const auto alternate_walls = storage.getSettingBoolean("material_alternate_walls");
    ...
    for (const PathOrderPath<const ExtrusionLine*>& path : order_optimizer.paths)
    {
        ...
        size_t start_index = (backwards != path.backwards) ? path.vertices->size() - (path.start_vertex + 1) : path.start_vertex;
        if (path.is_closed)
            start_index = path.start_vertex;
        ...
        gcode_layer.addWall(*path.vertices, start_index, settings, non_bridge_config, bridge_config, wipe_dist, flow, retract_before, path.is_closed, backwards, linked_path, skin_position_type);
        added_something = true;
    }
    return added_something;
}

std::set<std::pair<const ExtrusionLine*, const ExtrusionLine*>> InsetOrderOptimizer::getRegionOrder(const std::vector<const ExtrusionLine*>& input, const bool outer_to_inner)
{
    std::set<std::pair<const ExtrusionLine*, const ExtrusionLine*>> order_requirements;
...
std::set<std::pair<const ExtrusionLine*, const ExtrusionLine*>> InsetOrderOptimizer::getInsetOrder(const std::vector<const ExtrusionLine*>& input, const bool outer_to_inner)
{
    std::set<std::pair<const ExtrusionLine*, const ExtrusionLine*>> order;
...
bool InsetOrderOptimizer::shouldReversePath(const bool use_one_extruder, const bool current_extruder_is_wall_x, const bool outer_to_inner)
{
