...
GCodePath* LayerPlan::getLatestPathWithConfig(const GCodePathConfig& config, SpaceFillType space_fill_type, float flow, float width_factor, bool spiralize, double speed_factor)
{
    std::vector<GCodePath>& paths = extruder_plans.back().paths;
    if (paths.size() > 0 && paths.back().config == &config && !paths.back().done && paths.back().flow == flow && paths.back().width_factor == width_factor && paths.back().speed_factor == speed_factor) // spiralize can only change when a travel path is in between
        return &paths.back();
    paths.emplace_back(config, space_fill_type, flow, width_factor, spiralize, speed_factor);
...
LayerPlan::LayerPlan(const FffGcodeWriter& gcode_writer, const SliceDataStorage& storage, int layer_nr, int z, int layer_thickness, unsigned int start_extruder_this_layer, const std::vector<FanSpeedLayerTimeSettings>& fan_speed_layer_time_settings_per_extruder, CombingMode combing_mode, int64_t comb_boundary_offset, bool travel_avoid_other_parts, int64_t travel_avoid_distance)
...
, last_extruder_previous_layer(start_extruder_this_layer)
, last_planned_extruder_setting_base(storage.meshgroup->getExtruderTrain(start_extruder_this_layer))
...
, has_Interlaced_prime_tower_planned(false)
, has_prime_tower_finished(false)
{
    int current_extruder = start_extruder_this_layer;
    comb = nullptr;
...
{
    if (extruder == getExtruder()) //Get the last planned extruder.
    {
        return false;
    }
    setIsInside(false);
    { // handle end position of the prev extruder
        SettingsBaseVirtual* train = getLastPlannedExtruderTrainSettings(); //得到上一个ExtruderTrainSettings;
    ...
    {
        if((int)extruder_plans.size() < storage.meshgroup->getExtruderCount())
...
void LayerPlan::addExtrusionMove(Point p, const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio & flow, const Ratio width_factor, const bool spiralize, const Ratio speed_factor, const double fan_speed, const SkinPositionType skin_position_type)
{
...
void LayerPlan::addWallLine(const Point& p0, const Point& p1, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, float flow, const Ratio width_factor, float& non_bridge_line_volume, Ratio speed_factor, double distance_to_bridge_start, const SkinPositionType skin_position_type)
{
    ...
    const double overhang_speed_factor = extr->getSettingAsRatio("wall_overhang_speed_factor");
    ...
    auto addNonBridgeLine = [&](const Point& line_end)
    {
        ...
        while (distance_to_line_end > min_line_len)
        {
            ...
            if (distance_to_bridge_start > 0)
            {
                ...
                if (coast_dist > 0 && ((distance_to_bridge_start - len) <= coast_dist))
                {
                    if ((len - coast_dist) > min_line_len)
                    {
                        ...
                        addExtrusionMove(segment_end + coast_dist * (cur_point - segment_end) / len, non_bridge_config, SpaceFillType::Polygons, flow, width_factor, spiralize, speed_factor, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
                    }
                    // then coast to start of bridge segment
                    constexpr Ratio coast_flow = 0.0_r;
                    addExtrusionMove(segment_end, non_bridge_config, SpaceFillType::Polygons, coast_flow, width_factor, spiralize, speed_factor, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
                }
                else
                {
                    // no coasting required, just normal segment using non-bridge config
                    addExtrusionMove(segment_end, non_bridge_config, SpaceFillType::Polygons, flow, width_factor, spiralize, speed_factor, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
                }
            ...
            non_bridge_line_volume += vSize(cur_point - segment_end) * flow * width_factor * speed_factor * non_bridge_config.getSpeed();
    ...
    {
        // no bridges required
        addExtrusionMove(p1, non_bridge_config, SpaceFillType::Polygons, flow, width_factor, spiralize, speed_factor, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
    }
    else
    {
        ...
        if (PolygonUtils::polygonCollidesWithLineSegment(bridge_wall_mask, p0, p1))
        {
            ...
            while (line_polys.size() > 0)
            {
                ...
                if (bridge_line_len >= min_bridge_line_len)
                {
                    ...
                    {
                        addExtrusionMove(b1, bridge_config, SpaceFillType::Polygons, flow, width_factor, false, 1.0, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
        ...
        else if (bridge_wall_mask.inside(p0, true) && vSize(p0 - p1) >= min_bridge_line_len)
        {
            ...
            addExtrusionMove(p1, bridge_config, SpaceFillType::Polygons, flow, width_factor, false, 1.0, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
            non_bridge_line_volume = 0;
        }
        else
        {
...
void LayerPlan::addWall(ConstPolygonRef wall, int start_idx, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract)
{
...
void LayerPlan::addWall(const ExtrusionLine& wall, int start_idx, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract, const bool is_closed, const bool is_reversed, const bool is_linked_path, const SkinPositionType skin_position_type)
{
    if (wall.empty()){
        return;
    }
    ...
    const coord_t min_bridge_line_len = settings.getSettingInMicrons("bridge_wall_min_length");
    ...
    const coord_t small_feature_max_length_level_0 = settings.getSettingInMicrons("small_feature_max_length");
    const bool is_small_feature_level_0 = (small_feature_max_length_level_0 > 0) && cura::shorterThan(wall, small_feature_max_length_level_0);
    Ratio small_feature_speed_factor_level_0 = settings.getSettingAsRatio((layer_nr == 0) ? "small_feature_speed_factor_0" : "small_feature_speed_factor");

    const coord_t small_feature_max_length_level_1 = settings.getSettingInMicrons("small_feature_max_length_level_1");
    const bool is_small_feature_level_1 = (small_feature_max_length_level_1 > 0) && cura::shorterThan(wall, small_feature_max_length_level_1);
    Ratio small_feature_speed_factor_level_1 = settings.getSettingAsRatio((layer_nr == 0) ? "small_feature_speed_factor_0" : "small_feature_speed_factor_level_1");

    const coord_t small_feature_max_length_level_2 = settings.getSettingInMicrons("small_feature_max_length_level_2");
    const bool is_small_feature_level_2 = (small_feature_max_length_level_2 > 0) && cura::shorterThan(wall, small_feature_max_length_level_2);
    Ratio small_feature_speed_factor_level_2 = settings.getSettingAsRatio((layer_nr == 0) ? "small_feature_speed_factor_0" : "small_feature_speed_factor_level_2");

    bool is_small_feature = false;
    Ratio small_feature_speed_factor = 1.0f;
    if (is_small_feature_level_0){
        is_small_feature = true;
        small_feature_speed_factor = small_feature_speed_factor_level_0;
    }
    else if (small_feature_max_length_level_1){
        is_small_feature = true;
        small_feature_speed_factor = small_feature_speed_factor_level_1;
    }
    else if (small_feature_max_length_level_2) {
        is_small_feature = true;
        small_feature_speed_factor = small_feature_speed_factor_level_2;
    }

    const Velocity min_speed = fan_speed_layer_time_settings_per_extruder[extruder_plans.back().extruder].cool_min_speed;
    small_feature_speed_factor_level_0 = std::max((double)small_feature_speed_factor_level_0, (double)(min_speed / non_bridge_config.getSpeed()));
    small_feature_speed_factor_level_1 = std::max((double)small_feature_speed_factor_level_1, (double)(min_speed / non_bridge_config.getSpeed()));
    small_feature_speed_factor_level_2 = std::max((double)small_feature_speed_factor_level_2, (double)(min_speed / non_bridge_config.getSpeed()));
    const coord_t max_area_deviation = std::max(settings.getSettingAsCount("meshfix_maximum_extrusion_area_deviation"), 1); //Square micrometres!
    const coord_t max_resolution = std::max(settings.getSettingAsCount("meshfix_maximum_resolution"), 1);
        ...
        */
        ...
        for (size_t piece = 0; piece < pieces; ++piece)
        {
            ...
            if (is_small_feature)
            {
                ...
                addExtrusionMove(destination, non_bridge_config, SpaceFillType::Polygons, flow_ratio, line_width * nominal_line_width_multiplier, spiralize, small_feature_speed_factor, GCodePathConfig::FAN_SPEED_DEFAULT, skin_position_type);
            }
            else
            {
                const Point origin = p0.p + normal(line_vector, piece_length * piece);
                addWallLine(origin, destination, settings, non_bridge_config, bridge_config, flow_ratio, line_width * nominal_line_width_multiplier, non_bridge_line_volume, speed_factor, distance_to_bridge_start, skin_position_type);
            }
        }
        p0 = p1;
    }
...
)
{
    ...
    if (enable_travel_optimization && comb_boundary_inside.size() > 0)
    {
        ...
        if (layer_nr >= 0)
        {
            ...
            for (const SliceMeshStorage& mesh : storage.meshes)
            {
				coord_t skin_overlap_mm = mesh.getSettingInMicrons("skin_top_overlap_mm");
				for (const SliceLayerPart& part : mesh.layers[layer_nr].parts)
				{
					for (const SkinPart& skin_part : part.skin_parts)
					{
						...
						{
							skin_overlap_mm = mesh.getSettingInMicrons("skin_bottom_overlap_mm");
							break;
						}
					}
				}

                int overlap = std::max(skin_overlap_mm, mesh.getSettingInMicrons("infill_overlap_mm"));
                if (overlap > dist)
                {
                    dist = overlap;
                }
    ...
    const double fan_speed)
{
    ...
    std::optional<Point> near_start_location = std::optional<Point>();
    near_start_location = getLastPlannedPositionOrStartingPosition();
    addLinesByOptimizer(left_over, config, space_fill_type, true, wipe_dist, flow_ratio, near_start_location, fan_speed);
}
...
void ExtruderPlan::processFanSpeedAndMinimalLayerTime(bool force_minimal_layer_time, Point starting_position)
{
	...
	force_minimal_layer_time = force_minimal_layer_time && fan_speed_layer_time_settings.enable_auto_control;
    ...
    */
    ...
    fan_speed = fan_speed_layer_time_settings.cool_fan_speed_min; //Regular Fan Speed
...
void LayerPlan::writeGCode(GCodeExport& gcode)
{
    ...
    cura::Point temp_last_position = gcode.getPositionXY();
    ...
    for(unsigned int extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        if (extruder != extruder_plan.extruder)
        {
            ...
            { // require printing temperature to be met
				...
				double required_start_temperature = extruder_plan.required_start_temperature;
				if (!gcode.getIsExtruderPreheat(extruder))
				{
					required_start_temperature = train->getSettingInDegreeCelsius("material_initial_print_temperature");
				}

                if (this->is_raft_layer)
                {
                    assert(print_temperature_per_extruder.size() == 2);
                    required_start_temperature = print_temperature_per_extruder[1];
                }

                gcode.writeTemperatureCommand(extruder, required_start_temperature, wait);
				gcode.TK_ExtraE_Brush();//start gcode
            }
            ...
            { // turn off previous extruder
				...
				if (!gcode.getIsExtruderPreheat(prev_extruder))
				{
					const ExtruderTrain* prev_train = storage.meshgroup->getExtruderTrain(prev_extruder);
					prev_extruder_temp = prev_train->getSettingInSeconds("material_standby_temperature");
				}

				if (prev_layer_nr == storage.max_print_height_per_extruder[prev_extruder] || storage.max_print_height_per_extruder[prev_extruder] <= layer_nr)
				{
        ...
        if (this->is_raft_layer && extruder_plans.size() == 1)
        {
            assert(print_temperature_per_extruder.size() == 1);
            gcode.writeTemperatureCommand(extruder_plan.extruder, print_temperature_per_extruder[0], true);
        }
        ...
        if (extruder_plan.paths.size() > 0 && !extruder_plan.paths[0].config->isTravelPath())
        {
            if (extruder_plan.paths[0].getFanSpeed() != GCodePathConfig::FAN_SPEED_DEFAULT)
                gcode.writeFanCommand(extruder_plan.paths[0].getFanSpeed(), extruder);
            else
                gcode.writeFanCommand(extruder_plan.getFanSpeed(), extruder);
        }
		...
		int nozzle_offset_z = train->getSettingInMicrons("machine_nozzle_offset_z");

		double layer_height = storage.meshgroup->getSettingInMicrons("layer_height");

		int nozzle_offset_overlap = (int)(layer_height * train->getSettingInMillimeters("z_offset_overlap_coefficient"));
        ...
        for(unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            const GCodePath* prev_path = (path_idx == 0) ?  nullptr : &paths[path_idx-1];
            const GCodePath* next_path = (path_idx == paths.size()-1) ? nullptr : &paths[path_idx+1];

            bool path_is_closed = false;

			int pre_current_layer_z = gcode.getZ();

			if (layer_nr >= -Raft::getFillerLayerCount(storage))
			{
				if (path.config->type == PrintFeatureType::MoveCombing || path.config->type == PrintFeatureType::SupportInfill)
					gcode.setZ(z + nozzle_offset_z);
				else
					gcode.setZ(z + nozzle_offset_z - nozzle_offset_overlap);
			}
            ...
            {
                gcode.writeTypeComment(path.config->type);	//写;TYPE:WALL-OUTER
            ...
            if (path.isOverrideSpeedParameter())
                speed = path.getOverrideSpeed();
            ...
            if (path.config->isTravelPath())	//G0---判断line_width的值是否等于0
            { // early comp for travel paths, which are handled more simply
                for(unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                {
					gcode.writeTravel(path.points[point_idx], speed);
                }
                temp_last_position = path.GetLastPoint();
                continue;
            }
            ...
            if (!spiralize) // normal (extrusion) move (with coasting
            {
                ...
                double path_fan_speed = path.config->getFanSpeed();

                if (path.isOverrideFanSpeedParameter())
                    path_fan_speed = path.getFanSpeed();

                gcode.writeFanCommand(path_fan_speed >= 0 ? path_fan_speed : extruder_plan.getFanSpeed(), extruder);
                ...
                { // normal path to gcode algorithm
                    ...
                    {
                        sendLineTo(paths[path_idx+2].config->type, paths[path_idx+2].points.back(), paths[path_idx+2].getLineWidthForLayerView(), paths[path_idx+2].config->getLayerThickness(), speed, gcode.getCurrentFanSpeed());
                    ...
                    else 
                    {
                        for(unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                        {
                            double used_speed = path.getRevisedLineSpeed(point_idx)>0 ? std::min(path.getRevisedLineSpeed(point_idx), speed) : speed;
                            sendLineTo(path.config->type, path.points[point_idx], path.getLineWidthForLayerView(), path.config->getLayerThickness(), used_speed, gcode.getCurrentFanSpeed());
							int z_offset = 0;
							if (path.config->type == PrintFeatureType::SupportRoofInterface && layer_nr >= -Raft::getFillerLayerCount(storage) && path.config->getLayerThickness() != layer_height)
								z_offset = layer_height - path.config->getLayerThickness();

                            int integer_speed = int(used_speed + 0.5);
                            gcode.writeExtrusion(path.points[point_idx], integer_speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset, z_offset);
                        }
                    }
                }
            }
            ...
            { // SPIRALIZE
                ...
                { // handle all consecutive spiralized paths > CHANGES path_idx!
                    ...
                    for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                    {
                        ...
                        int integer_speed = int(used_speed + 0.5);
                        gcode.writeExtrusion(path.points[point_idx], integer_speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset);
                    }
                    ...
                    sendLineTo(path.config->type, path.points[0], path.getLineWidthForLayerView(), path.config->getLayerThickness(), speed, gcode.getCurrentFanSpeed());
                }
        ...
        if (extruder_plan.extruder_start_gcode_time)
        {
            double dbExtraTime = *extruder_plan.extruder_start_gcode_time;
            assert(dbExtraTime>=0);
            gcode.AddExtraTime(dbExtraTime);
        }
        if (extruder_plan.extruder_end_gcode_time)
        {
            double dbExtraTime = *extruder_plan.extruder_end_gcode_time;
            assert(dbExtraTime >= 0);
            gcode.AddExtraTime(dbExtraTime);
        }

		if (gcode.getExtruderLayerCleanupEnable(extruder))
		{
			gcode.addExtruderLayerCleanupNum(extruder);
		}
    } // extruder plans /\  .
...
bool LayerPlan::writePathWithCoasting(GCodeExport& gcode, unsigned int extruder_plan_idx, unsigned int path_idx, int64_t layerThickness, double coasting_volume, double coasting_speed, double coasting_min_volume)
{
    ...
    { // write normal extrude path:
        ...
        {
            double used_speed = path.getRevisedLineSpeed(point_idx) > 0 ? std::min(path.getRevisedLineSpeed(point_idx), extrude_speed) : extrude_speed;

            sendLineTo(path.config->type, path.points[point_idx], path.getLineWidthForLayerView(), path.config->getLayerThickness(), used_speed, gcode.getCurrentFanSpeed());
            gcode.writeExtrusion(path.points[point_idx], used_speed, path.getExtrusionMM3perMM(), path.config->type);
        }
        sendLineTo(path.config->type, start, path.getLineWidthForLayerView(), path.config->getLayerThickness(), extrude_speed, gcode.getCurrentFanSpeed());
...
)
{
    ...
    for (size_t order_idx = 0; order_idx < paths.size(); order_idx++)
    {
        ...
        if (vSize2(getLastPlannedPositionOrStartingPosition() - start) < line_width_2 * 0.5)
        {
