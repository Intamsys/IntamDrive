...
public:
    ...
    std::optional<double> prev_extruder_used_time;
    std::optional<double> pre_heated_starting_temperature;
    std::optional<double> pre_heated_during_time;
    std::optional<double> this_end_temperature; 
    std::optional<double> other_end_temperature;

    std::optional<double> this_start_temperature;
    std::optional<double> other_start_temperature;

    std::optional<double> extruder_end_gcode_time;
    std::optional<double> extruder_start_gcode_time;
...
public:
    ...
    double getAlreadyUsedTime()
    {
        return already_used_sum_time;
    }

    NozzleTempInsert getLastTempInsert()
    {
        if (inserts.size() > 0)
        {
            return inserts.back();
...
private:
    ...
    const std::vector<double> print_temperature_per_extruder;

	bool has_Interlaced_prime_tower_planned;

    bool has_prime_tower_finished;
...
private:
     ...
     */
    GCodePath* getLatestPathWithConfig(const GCodePathConfig& config, SpaceFillType space_fill_type, float flow = 1.0, float width_factor = 1.0, bool spiralize = false, double speed_factor = 1.0);
...
public:
     ...
     */
    LayerPlan(const FffGcodeWriter& gcode_writer, const SliceDataStorage& storage, int layer_nr, int z, int layer_height, unsigned int start_extruder_this_layer, const std::vector<FanSpeedLayerTimeSettings>& fan_speed_layer_time_settings_per_extruder, CombingMode combing_mode, int64_t comb_boundary_offset, bool travel_avoid_other_parts, int64_t travel_avoid_distance);
...
public:
	...
	{
		return has_Interlaced_prime_tower_planned;
	}

	void setPrimeTowerIsPlanned()
	{
		has_Interlaced_prime_tower_planned = true;
	}

    bool getPrimeTowerIsFinished() const
    {
        return has_prime_tower_finished;
    }
    
    void setPrimeTowerFinished()
    {
        has_prime_tower_finished = true;
    }
     ...
     */
    void sendLineTo(PrintFeatureType print_feature_type, Point to, int line_width, int line_thickness, int line_feedrate, int line_fan_speed) const
    {
		if(print_feature_type == PrintFeatureType::SupportRoofInterface || print_feature_type == PrintFeatureType::SupportBottomInterface)
			CommandSocket::sendLineTo(PrintFeatureType::SupportInterface, to, line_width, line_thickness, line_feedrate, line_fan_speed);
		else
			CommandSocket::sendLineTo(print_feature_type, to, line_width, line_thickness, line_feedrate, line_fan_speed);
    }
    ...
    */
     ...
     * \param spiralize Whether to gradually increase the z while printing. (Note that this path may be part of a sequence of spiralized paths, forming one polygon)
     */
    void addExtrusionMove(Point p, const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio& flow = 1.0_r, const Ratio width_factor = 1.0_r, const bool spiralize = false, const Ratio speed_factor = 1.0_r, const double fan_speed = GCodePathConfig::FAN_SPEED_DEFAULT, const SkinPositionType skin_position_type = SkinPositionType::NO_SKIN);
     ...
     */
    void addWallLine(const Point& p0, const Point& p1, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, float flow, const Ratio width_factor, float& non_bridge_line_volume, Ratio speed_factor, double distance_to_bridge_start, const SkinPositionType skin_position_type = SkinPositionType::NO_SKIN);
...
/*!
     ...
     */
    void addWall(ConstPolygonRef polygon, int start_idx, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract);
    /*!
     ...
     */
    void addWall(const ExtrusionLine& wall, int start_idx, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract, const bool is_closed, const bool is_reversed, const bool is_linked_path, const SkinPositionType skin_position_type = SkinPositionType::NO_SKIN);

    void addWalls(const Polygons& walls, const SettingsBaseVirtual& settings, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, const ZSeamConfig& z_seam_config = ZSeamConfig(), coord_t wall_0_wipe_dist = 0, float flow_ratio = 1.0, bool always_retract = false);
