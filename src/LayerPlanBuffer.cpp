...
void LayerPlanBuffer::setPreheatConfig(MeshGroup& settings)
{
    ...
    for (int extruder_nr = 0; extruder_nr < settings.getExtruderCount(); extruder_nr++)
    {
        ...
        const ExtruderTrain& extruder_train = *settings.getExtruderTrain(extruder_nr);
        Point end_pos(extruder_train.getSettingInMicrons("machine_extruder_end_pos_x"), extruder_train.getSettingInMicrons("machine_extruder_end_pos_y"));
        end_point_per_extruder.push_back(end_pos);
    }
}
 ...
 //   assert(newest_layer->extruder_plans.front().extruder == prev_layer->extruder_plans.back().extruder);
    ...
    if (!prev_layer->last_planned_position || *prev_layer->last_planned_position != first_location_new_layer)
    {
        ...
        const bool force_retract = prev_layer->storage.getSettingBoolean("retract_at_layer_change") ||
          (prev_layer->storage.getSettingBoolean("travel_retract_before_outer_wall") && (prev_layer->storage.getSettingsAsInsetDirection("inset_direction")== InsetDirection::OUTSIDE_IN || prev_layer->storage.getSettingAsCount("wall_line_count") == 1)); //Moving towards an outer wall.
...
void LayerPlanBuffer::insertPreheatCommand(ExtruderPlan& extruder_plan_current, ExtruderPlan& extruder_plan_before, double time_after_extruder_plan_start, int extruder, double temp, double sum_time_before_extruder_plan)
{
    double acc_time = 0.0;
    for (unsigned int path_idx = extruder_plan_before.paths.size() - 1; int(path_idx) != -1 ; path_idx--)
    {
        ...
        if (acc_time > time_after_extruder_plan_start)
        {
            ...
            bool wait = false;
            extruder_plan_before.insertCommand(path_idx, extruder, temp, wait, gcode.getIsExtruderPreheat(extruder), time_this_path - time_before_path_end);
            
            if (extruder_plan_current.extruder != extruder_plan_before.extruder)
            {
                extruder_plan_current.pre_heated_during_time = sum_time_before_extruder_plan + acc_time;
            }
            return;
        }
    }
    ...
    extruder_plan_before.insertCommand(path_idx, extruder, temp, wait, gcode.getIsExtruderPreheat(extruder)); // insert at start of extruder plan if time_after_extruder_plan_start > extruder_plan.time
    if (extruder_plan_current.extruder != extruder_plan_before.extruder)
    {
        extruder_plan_current.pre_heated_during_time = sum_time_before_extruder_plan + acc_time;
    }
...
Preheat::WarmUpResult LayerPlanBuffer::computeStandbyTempPlan(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
    ...
    { // find a previous extruder plan where the same extruder is used to see what time this extruder wasn't used
        ...
        {
            double  dbPrevUsedTime = *(extruder_plan.prev_extruder_used_time);
            double  dbDiffTime = dbPrevUsedTime - in_between_time;
            ...
            if (extruder_plan_before.this_end_temperature)
            {
                temp_before = *(extruder_plan_before.this_end_temperature);
            }
            ExtruderPlan& extruder_plan_before_1 = *extruder_plans[extruder_plan_before_idx+1];
            if (extruder_plan_before_1.other_start_temperature)
            {
                temp_before = *(extruder_plan_before_1.other_start_temperature);
            }
    ...
    double dbOutTemp;
    {
        double dbEndGCodeTemp = gcode.getExtruderFinalTemp(extruder);
        double dbLeftTime = *(extruder_plan.prev_extruder_used_time) - in_between_time;
        dbOutTemp = dbEndGCodeTemp - dbLeftTime * preheat_config.getCooldownSpeed(extruder, false);
        if(dbOutTemp < preheat_config.getStandbyTemp(extruder))
            dbOutTemp = preheat_config.getStandbyTemp(extruder);
    }
    constexpr bool during_printing = false;
    Preheat::WarmUpResult warm_up = preheat_config.getWarmUpPointAfterCoolDown(in_between_time, extruder, dbOutTemp, preheat_config.getStandbyTemp(extruder), initial_print_temp, during_printing);
    warm_up.heating_time = std::min(in_between_time, warm_up.heating_time + extra_preheat_time);
    return warm_up;
}

double LayerPlanBuffer::computePreheatedStartingTemp(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
    ExtruderPlan& extruder_plan = *extruder_plans[extruder_plan_idx];
    int extruder = extruder_plan.extruder;
    ...
    { // find a previous extruder plan where the same extruder is used to see what time this extruder wasn't used
        ...
        if (extruder_plan_before.extruder == extruder)
        {
            if (extruder_plan_before.this_end_temperature)
            {
                return *(extruder_plan_before.this_end_temperature);
            }
        }
    }
    return preheat_config.getStandbyTemp(extruder);
}

void LayerPlanBuffer::insertPreheatCommand_singleExtrusion(ExtruderPlan& current_extruder_plan, ExtruderPlan& prev_extruder_plan, int extruder, double required_temp)
{
    ...
    {
        time_before_extruder_plan_end = 0;
    }

    insertPreheatCommand(current_extruder_plan, prev_extruder_plan, time_before_extruder_plan_end, extruder, required_temp, 0);
}
...
void LayerPlanBuffer::handleStandbyTemp(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx, double standby_temp)
{
    ...
    earliest_extruder_plan.insertCommand(0, extruder, standby_temp, wait, gcode.getIsExtruderPreheat(extruder));
}
...
void LayerPlanBuffer::insertPreheatCommand_multiExtrusion(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
    ...
    const int prev_extruder = prev_extruder_plan.extruder;

    if (!gcode.getIsExtruderPreheat(extruder))
    {
        insertPreheatCommand(extruder_plan, prev_extruder_plan, 0, extruder, extruder_plan.required_start_temperature, 0);
        return;
    }
    ...
    else if (heating_time_and_from_temp.heating_time + 1e-3 < heating_time_and_from_temp.total_time_window)
    ...
    double sum_time_before_extruder_plan = 0;
    ...
    for (unsigned int extruder_plan_before_idx = extruder_plan_idx - 1; int(extruder_plan_before_idx) >= 0; extruder_plan_before_idx--)
    {
        ...
        {
            ExtruderPlan& extruder_plan_before_1 = *extruder_plans[extruder_plan_before_idx+1];
            extruder_plan.pre_heated_starting_temperature = heating_time_and_from_temp.lowest_temperature;
            insertPreheatCommand(extruder_plan, extruder_plan_before_1, 0, extruder, initial_print_temp, sum_time_before_extruder_plan);
            return;
        }
        else
        {
            double time_here = extruder_plan_before.estimates.getTotalTime();
            if (time_here + 1e-6 >= time_before_extruder_plan_to_insert)
            {
                insertPreheatCommand(extruder_plan, extruder_plan_before, time_before_extruder_plan_to_insert, extruder, initial_print_temp, sum_time_before_extruder_plan);
                return;
            }
            time_before_extruder_plan_to_insert -= time_here;
            sum_time_before_extruder_plan += time_here;
        }
    }
    // time_before_extruder_plan_to_insert falls before all plans in the buffer
    bool wait = false;
    unsigned int path_idx = 0;
    extruder_plans[0]->insertCommand(path_idx, extruder, initial_print_temp, wait, gcode.getIsExtruderPreheat(extruder)); // insert preheat command at verfy beginning of buffer
    
    if (extruder_plan.prev_extruder_used_time)
    {
        extruder_plan.pre_heated_during_time = sum_time_before_extruder_plan;
        extruder_plan.pre_heated_starting_temperature = preheat_config.getStandbyTemp(extruder);
    }
}
...
{   
    ...
    if (prev_extruder == extruder)
    {
        extruder_plan.already_used_sum_time = prev_extruder_plan->already_used_sum_time + extruder_plan.estimates.getTotalTime();
        insertPreheatCommand_singleExtrusion(extruder_plan, *prev_extruder_plan, extruder, extruder_plan.required_start_temperature);

        if (!prev_extruder_plan->inserts.empty())
        {
    ...
    {
        extruder_plan.already_used_sum_time = extruder_plan.estimates.getTotalTime();
        extruder_plan.prev_extruder_used_time = prev_extruder_plan->already_used_sum_time;
...
void LayerPlanBuffer::insertPrintTempCommand(ExtruderPlan& extruder_plan)
{
    ...
    { // handle heating from initial_print_temperature to printing_tempreature
        ...
        bool wait = false;
        extruder_plan.insertCommand(path_idx, extruder, print_temp, wait, gcode.getIsExtruderPreheat(extruder));
    }
...
void LayerPlanBuffer::insertFinalPrintTempCommand(std::vector<ExtruderPlan*>& extruder_plans, unsigned int last_extruder_plan_idx)
{
    ...
    if (time_window > -1e-6 && time_window < 0)
        time_window = 0.0;
    ...
    { // insert temp command in precool_extruder_plan
        ...
        precool_extruder_plan->insertCommand(path_idx, extruder, final_print_temp, wait, gcode.getIsExtruderPreheat(extruder), time_after_path_start);
    }
}
...
void LayerPlanBuffer::insertTempCommands()
{
    ...
    for (unsigned int extruder_plan_idx = 0; extruder_plan_idx < layer_plan.extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        { // the very first extruder plan of the current meshgroup
            ...
            extruder_plan.already_used_sum_time = extruder_plan.totalPrintTime;
            continue;
        }
