...
class LayerPlanBuffer : SettingsMessenger
{
    ...
    std::vector<Point> end_point_per_extruder;
...
private:
     ...
     */
    void insertPreheatCommand(ExtruderPlan& extruder_plan_current, ExtruderPlan& extruder_plan_before, double time_before_extruder_plan_end, int extruder, double temp, double sum_time_before_extruder_plan);
    ...
    double computePreheatedStartingTemp(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx);
     ...
     */
    void insertPreheatCommand_singleExtrusion(ExtruderPlan& current_extruder_plan, ExtruderPlan& prev_extruder_plan, int extruder, double required_temp);
