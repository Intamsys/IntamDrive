...
private:
	...
	ExtrusionMoves extra_pattern;
    
    bool first_layer_done;
...
public:
     ...
     */
    void addToGcode(const SliceDataStorage& storage, LayerPlan& gcode_layer, const GCodeExport& gcode, const int prev_extruder, const int new_extruder, const bool print_all = false) const;
	...
	void UpdateExtruderOrder(SliceDataStorage& storage);

	void Init(const SliceDataStorage& storage);
...
private:
	...
	*/
	...
	void addOtherToGCode_denseInfill(const SliceDataStorage& storage, LayerPlan& gcode_layer, const int extruder_nr, const int other_extruder) const;
};
