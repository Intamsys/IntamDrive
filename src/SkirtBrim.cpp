...
void SkirtBrim::getFirstLayerOutline(SliceDataStorage& storage, const unsigned int primary_line_count, const int primary_extruder_skirt_brim_line_width, const bool is_skirt, const bool outside_only, Polygons& first_layer_outline)
{
    ...
    { // add brim underneath support by removing support where there's brim around the model
        ...
        if (storage.getSettingAsPrimeTowerType("prime_tower_type") == PrimeTowerType::NESTED && storage.primeTower.enabled)
        {
            first_layer_outline.add(storage.primeTower.outer_poly); // don't remove parts of the prime tower, but make a brim for it
        }
		if (storage.getSettingAsPrimeTowerType("prime_tower_type") == PrimeTowerType::INTERLACED && storage.interlacedprimeTower.enabled)
		{
			first_layer_outline.add(storage.interlacedprimeTower.ground_poly);
		}
    }
