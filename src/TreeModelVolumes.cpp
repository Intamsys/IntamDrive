...
TreeModelVolumes::TreeModelVolumes(const SliceDataStorage& storage)
    ...
    , xy_distance_(storage.getSettingInMicrons("support_xy_distance"))
    , xy_distance_overhang(storage.getSettingInMicrons("support_xy_distance_overhang"))
    , distance_priority(storage.getSettingAsSupportDistPriority("support_xy_overrides_z"))
    , radius_sample_resolution_(storage.getSettingInMicrons("support_tree_collision_resolution"))
{
    const coord_t layer_height = storage.getSettingInMicrons("layer_height");
    const AngleRadians angle = storage.getSettingInAngleRadians("support_tree_angle");
    max_move_ = (angle < TAU / 4) ? (coord_t)(tan(angle) * layer_height) : std::numeric_limits<coord_t>::max();
    z_distance_layers = round_up_divide(storage.getSettingInMicrons("support_top_distance"), layer_height) + 1;
