...
#include "settings/settings.h" //To store whether X/Y or Z distance gets priority.
...
public:
     ...
     */
    TreeModelVolumes(const SliceDataStorage& storage);
...
private:
     ...
     */
    mutable std::map<RadiusLayerPair, Polygons> collision_cache_;
    mutable std::map<RadiusLayerPair, Polygons> avoidance_cache_;
    mutable std::map<RadiusLayerPair, Polygons> internal_model_cache_;
};
