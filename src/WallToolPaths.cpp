...
WallToolPaths::WallToolPaths(const Polygons& outline, const coord_t nominal_bead_width, const size_t inset_count, const coord_t wall_0_inset,
    ...
    , print_thin_walls(settings.getSettingBoolean("fill_outline_gaps"))
    , min_feature_size(settings.getSettingInMicrons("min_feature_size"))
    , min_bead_width(settings.getSettingInMicrons("min_bead_width"))
...
WallToolPaths::WallToolPaths(const Polygons& outline, const coord_t bead_width_0, const coord_t bead_width_x,
                             const size_t inset_count, const coord_t wall_0_inset, const SettingsBaseVirtual& settings)
    : outline(outline)
...
const std::vector<VariableWidthLines>& WallToolPaths::generate(const SettingsBaseVirtual& settings)
{
    const coord_t smallest_segment = settings.getSettingInMicrons("meshfix_maximum_resolution");
    const coord_t allowed_distance = settings.getSettingInMicrons("meshfix_maximum_deviation");
    const coord_t open_close_distance = settings.getSettingBoolean("fill_outline_gaps") ? settings.getSettingInMicrons("min_feature_size") / 2 - 5 : settings.getSettingInMicrons("min_even_wall_line_width") / 2 - 5;

    const coord_t epsilon_offset = (allowed_distance / 2) - 1;
    const AngleRadians transitioning_angle = settings.getSettingInAngleRadians("wall_transition_angle");
    ...
    const coord_t wall_transition_length = settings.getSettingInMicrons("wall_transition_length");

    //when to split the middle wall into two:
    const double min_even_wall_line_width = settings.getSettingInMillimeters("min_even_wall_line_width");
    const double wall_line_width_0 = settings.getSettingInMillimeters("wall_line_width_0");
    ...
    const double min_odd_wall_line_width = settings.getSettingInMillimeters("min_odd_wall_line_width");
    const double wall_line_width_x = settings.getSettingInMillimeters("wall_line_width_x");
    const Ratio wall_add_middle_threshold = std::max(1.0, std::min(99.0, 100.0 * min_odd_wall_line_width / wall_line_width_x)) / 100.0;

    const int wall_distribution_count = settings.getSettingAsCount("wall_distribution_count");
    ...
    const coord_t transition_filter_dist = settings.getSettingInMicrons("wall_transition_filter_distance");
    const coord_t allowed_filter_deviation = settings.getSettingInMicrons("wall_transition_filter_deviation");
    ...
    //simplifyAdjacentJunctions(toolpaths, settings);
...
void WallToolPaths::stitchToolPaths(std::vector<VariableWidthLines>& toolpaths, const SettingsBaseVirtual& mesh)
{
    const coord_t stitch_distance = mesh.getSettingInMicrons("wall_line_width_x") - 1; //In 0-width contours, junctions can cause up to 1-line-width gaps. Don't stitch more than 1 line width.
...
void WallToolPaths::simplifyAdjacentJunctions(std::vector<VariableWidthLines>& toolpaths, const SettingsBaseVirtual& settings)
{
    const coord_t maximum_resolution = settings.getSettingInMicrons("meshfix_maximum_resolution");
    const coord_t maximum_deviation = settings.getSettingInMicrons("meshfix_maximum_deviation");
    const coord_t maximum_extrusion_area_deviation = settings.getSettingAsCount("meshfix_maximum_extrusion_area_deviation");
    for (VariableWidthLines& inset : toolpaths)
    {
        for (size_t line_idx = 0; line_idx < inset.size(); line_idx++)
        {
            ExtrusionLine& line = inset[line_idx];
            if (!line.is_closed || line.junctions.size() < 4)
                continue;
            for (size_t junction_idx = 0; junction_idx < line.junctions.size(); junction_idx++)
            {
                ExtrusionJunction before_junction = line.junctions[(line.junctions.size() + junction_idx - 1) % line.junctions.size()];
                ExtrusionJunction this_junction = line.junctions[junction_idx];
                ExtrusionJunction after_junction = line.junctions[(junction_idx+1) % line.junctions.size()];
                ExtrusionJunction after_after_junction = line.junctions[(junction_idx + 2) % line.junctions.size()];

                coord_t this_line_length = vSize(this_junction.p - after_junction.p);
                coord_t after_line_length = vSize(after_after_junction.p - after_junction.p);
                coord_t before_line_length = vSize(this_junction.p - before_junction.p);

                if ((this_line_length < maximum_resolution / 10) ||
                    (std::abs(this_junction.w - this_junction.w) < std::min(this_junction.w, this_junction.w) / 10
                        && this_line_length < maximum_deviation 
                        && before_line_length / this_line_length > 10 
                        && after_line_length  / this_line_length > 10))
                {
                    line.junctions[(junction_idx + 1) % line.junctions.size()].p = Point((this_junction.p.X + after_junction.p.X)/2, (this_junction.p.Y + after_junction.p.Y) / 2);
                    line.junctions[(junction_idx + 1) % line.junctions.size()].w = (this_junction.w + after_junction.w) / 2;

                    line.junctions.erase(std::begin(line.junctions) + junction_idx); //Delete the second element
                    junction_idx--;
...
void WallToolPaths::simplifyToolPaths(std::vector<VariableWidthLines>& toolpaths, const SettingsBaseVirtual& mesh)
{
    const coord_t maximum_resolution = mesh.getSettingInMicrons("meshfix_maximum_resolution");
    const coord_t maximum_deviation = mesh.getSettingInMicrons("meshfix_maximum_deviation");
    const coord_t maximum_extrusion_area_deviation = mesh.getSettingAsCount("meshfix_maximum_extrusion_area_deviation");
    const Simplify simplifier(maximum_resolution, maximum_deviation, maximum_extrusion_area_deviation);
...
const std::vector<VariableWidthLines>& WallToolPaths::getToolPaths(const SettingsBaseVirtual& mesh)
{
...
void WallToolPaths::pushToolPaths(std::vector<VariableWidthLines>& paths, const SettingsBaseVirtual& settings)
{
    if (! toolpaths_generated)
    {
        generate(settings);
...
const Polygons& WallToolPaths::getInnerContour(const SettingsBaseVirtual& mesh)
{
