...
int bridgeAngle(const SettingsBaseVirtual& settings, const Polygons& skin_outline, const SliceDataStorage& storage, const unsigned layer_nr, const unsigned bridge_layer, const SupportLayer* support_layer, Polygons& supported_regions)
{
    ...
    const Ratio sparse_infill_max_density = settings.getSettingAsRatio("bridge_sparse_infill_max_density");
    ...
    for (const SliceMeshStorage& mesh : storage.meshes)
    {
        if (mesh.isPrinted())
        {
            ...
            const coord_t infill_line_width = mesh.getSettingInMicrons("infill_line_width");
    ...
    const bool bridge_settings_enabled = settings.getSettingBoolean("bridge_settings_enabled");
    const Ratio support_threshold = bridge_settings_enabled ? settings.getSettingAsRatio("bridge_skin_support_threshold") : 0.0_r;
