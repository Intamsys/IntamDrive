...
class CommandSocket::PathCompiler
{
    ...
    std::vector<float> line_fan_speeds; //!<Fan speed for line segments stored, the size of this vector is N.
    Point last_point;
...
public:
    ...
    void handleInitialPoint(Point from)
    {
        ...
        {
            addLineSegment(PrintFeatureType::NoneType, from, 1.0, 0.0, 0.0, 0.0);
        }
    }
     ...
     */
    void sendLineTo(PrintFeatureType print_feature_type, Point to, int width, int thickness, int feedrate, int fan_speed);
    /*!
     * Adds closed polygon to the current path
     */
    void sendPolygon(PrintFeatureType print_feature_type, ConstPolygonRef poly, int width, int thickness, int feedrate, int fan_speed);
private:
    /*!
     ...
     */
    void addLineSegment(PrintFeatureType print_feature_type, Point point, int line_width, int line_thickness, int line_feedrate, int fan_speed)
    {
        if (print_feature_type == PrintFeatureType::SkinRoof || print_feature_type == PrintFeatureType::SkinFloor)
            print_feature_type = PrintFeatureType::Skin;
        addPoint2D(point);
        ...
        line_fan_speeds.push_back(fan_speed);
    }
};
#endif
...
void CommandSocket::sendPolygons(PrintFeatureType type, const Polygons& polygons, int line_width, int line_thickness, int line_feedrate, int line_fan_speed)
{
    ...
    if (CommandSocket::isInstantiated())
    {
        ...
        {
            path_comp->sendPolygon(type, polygons[i], line_width, line_thickness, line_feedrate, line_fan_speed);
        }
    }
#endif
}

void CommandSocket::sendPolygon(PrintFeatureType type, ConstPolygonRef polygon, int line_width, int line_thickness, int line_feedrate, int line_fan_speed)
{
#ifdef ARCUS
    if (CommandSocket::isInstantiated())
    {
        ...
        path_comp->sendPolygon(type, polygon, line_width, line_thickness, line_feedrate, line_fan_speed);
    }
#endif
}

void CommandSocket::sendLineTo(cura::PrintFeatureType type, Point to, int line_width, int line_thickness, int line_feedrate, int line_fan_speed)
{
#ifdef ARCUS
    if (CommandSocket::isInstantiated())
    {
        ...
        path_comp->sendLineTo(type, to, line_width, line_thickness, line_feedrate, line_fan_speed);
    }
#endif
}
...
void CommandSocket::PathCompiler::flushPathSegments()
{
    if (line_types.size() > 0 && CommandSocket::isInstantiated())
    {
        ...
        std::string line_fan_speed_data;
        line_fan_speed_data.append(reinterpret_cast<const char*>(line_fan_speeds.data()), line_fan_speeds.size() * sizeof(float));
        p->set_line_fan_speed(line_fan_speed_data);
    ...
    line_widths.clear();
    line_types.clear();
    line_fan_speeds.clear();
}

void CommandSocket::PathCompiler::sendLineTo(PrintFeatureType print_feature_type, Point to, int width, int thickness, int feedrate, int fan_speed)
{
    assert(points.size() > 0 && "A point must already be in the buffer for sendLineTo(.) to function properly");

    if (to != last_point)
    {
        addLineSegment(print_feature_type, to, width, thickness, feedrate, fan_speed);
    }
}

void CommandSocket::PathCompiler::sendPolygon(PrintFeatureType print_feature_type, ConstPolygonRef polygon, int width, int thickness, int feedrate, int fan_speed)
{
    ...
    while (++it != it_end)
    {
        ...
        if (*it != last_point)
        {
            addLineSegment(print_feature_type, *it, width, thickness, feedrate, fan_speed);
        }
    }
    // Make sure the polygon is closed
    if (*polygon.begin() != polygon.back())
    {
        addLineSegment(print_feature_type, *polygon.begin(), width, thickness, feedrate, fan_speed);
    }
}
#endif
