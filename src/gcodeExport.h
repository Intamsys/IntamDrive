...
struct ExtruderHeatingStatus
{
    bool    heatandwaiting;
    int     extruder_nr;
    double  temperature;
    ExtruderHeatingStatus()
    {
        heatandwaiting = false;
        extruder_nr = -1;
        temperature = -100;
    }
};
...
private:
    struct ExtruderTrainAttributes
    {
		...
		std::string material_name; //!< The NAME for the material used by this extruder 
        std::string material_key;  //!< The KEY for the material used by this extruder

        std::string start_code;
        std::string end_code;

        bool is_forcing_cooling_after_used;
        int  final_temp;
        int  print_temp;
		...
		, nozzle_offset_z(0)
		, z_offset_overlap(0)
        , is_forcing_cooling_after_used(false)
        , final_temp(0)
        , print_temp(0)
        { }
    };
    ...
    double currentFanSpeed[MAX_EXTRUDERS];
    EGCodeFlavor flavor;
	...
	bool switch_first_start_Extruder;

	bool enable_extruder_clean_up[MAX_EXTRUDERS];
	double extruder_clean_up_threshold[MAX_EXTRUDERS];

	bool enable_extruder_layer_cleanup[MAX_EXTRUDERS];
	int interval_layer_num_cleanup[MAX_EXTRUDERS];

    bool enable_cleanup_extruder_before_Skin_layer[MAX_EXTRUDERS];

	std::string extruder_clean_up_code[MAX_EXTRUDERS];

	double accumulate_extrusion_value[MAX_EXTRUDERS];

	int	accumulate_layer_num_cleanup[MAX_EXTRUDERS];

	bool enable_extruder_preheat[MAX_EXTRUDERS];

	std::vector<RetractionConfig> retraction_config_per_extruder;

	bool enable_prime_tower;
	int prime_tower_height_layer;

	PrimeTowerHeightType prime_tower_high_type;

	double extruder_switch_time;
	int extruder_switch_count;

    int file_head_len;

    double build_plate_temperature;
    double chamber_temperature;

    ExtruderHeatingStatus lastest_extruder_heating_status;
...
public:
     ...
     */
    std::string getFileHeader(const std::vector<bool>& extruder_is_used, const double* print_time = nullptr, const std::vector<double>& filament_used = std::vector<double>(), const std::vector<double>& filament_weight = std::vector<double>(), const std::vector<std::string>& mat_ids = std::vector<std::string>(), const std::vector<std::string>& mat_keys = std::vector<std::string>());
    ...
    int getExtruderFinalTemp(const int extruder_nr) const;

    int getExtruderPrintTemp(const int extruder_nr) const;
	...
	std::string getMaterialNAME(const int extruder_nr) const; //!< returns the NAME of the material used for the nozzle with id \p extruder_nr 

    std::string getMaterialKEY(const int extruder_nr) const;  //!< returns the KEY of the material used for the nozzle with id \p extruder_nr

	Point getGcodePos(const int64_t x, const int64_t y, const int extruder_train) const;

	int getGcodeZOffset(const int extruder_nr) const;

	int getGcodeZOverlap(const int extruder_nr) const;
    ...
    void setFilamentDiameterAndDensity(unsigned int n, int diameter, int density);
    
    double getCurrentExtrudedVolume() const;

	bool getSwitchExtruderIsFirstIsUsed() const
	{
		return switch_first_start_Extruder;
	}
	
	void setRetraction(const int extruder_nr, const RetractionConfig& config);
    /*!
    ...
    double getTotalFilamentWeight(int extruder_nr);

	bool getIsExtruderPreheat(const int extruder_nr) const
	{
		return enable_extruder_preheat[extruder_nr];
	}
	...
	void SetStartingExtruder(const size_t start_extruder);
	...
	void AddExtraTime(double timeAmount);
     ...
     */
    void writeTravel(Point p, double speed, bool bforce = false);
     ...
     */
    void writeExtrusion(Point p, double speed, double extrusion_mm3_per_mm, PrintFeatureType feature, bool update_extrusion_offset = false, int z_offset = 0);
     ...
     * \param speed movement speed
     */
    void writeTravel(Point3 p, double speed, bool bforce = false);
	...
	void setExtruderRetractionConfig(std::vector<RetractionConfig>& extruder_retraction_config);

	void setPrimeTowerHeight(const SliceDataStorage& storage);

    void setBuildPlateAndChamberTemperture(double b_temperature, double c_temperature)
    {
        build_plate_temperature = b_temperature;
        chamber_temperature = c_temperature;
    }
private:
    /*!
     * Coordinates are build plate coordinates, which might be offsetted when extruder offsets are encoded in the gcode.
     ...
     * \param speed movement speed
     */
    void writeTravel(int x, int y, int z, double speed, bool bforce = false);
     ...
     */
    void writeFXYZE(double speed, int x, int y, int z, double e, PrintFeatureType feature, bool force_clean_up_extruder = false);
...
public:
     ...
     */
    void switchExtruder(const int new_extruder, const RetractionConfig& retraction_config_old_extruder);
    ...
    void writeFanCommand(double speed, int extruder);
    
    void writeTemperatureCommand(int extruder, double temperature, bool wait = false, bool startup = false, bool isPreheat = false);
	...
	void writeBuildplateT(double bedTemperature);

	void TK_ExtraE_Brush();

    double ComputeExtruderTemperature(const int extruder_nr, Preheat& preheat, const double dbTemp, const double dbTime, const bool isHeatupFlag);

	bool getExtruderLayerCleanupEnable(const int extruder_nr) const
	{
		return enable_extruder_layer_cleanup[extruder_nr];
	}

    bool getExtruderCleanupEnableBeforeSkinlayer(const int extruder_nr) const
    {
        return enable_cleanup_extruder_before_Skin_layer[extruder_nr];
    }

	void addExtruderLayerCleanupNum(int extruder_nr)
	{
		accumulate_layer_num_cleanup[extruder_nr]++;
	}

	void addExtruderSwitchTime(double dbCostTime)
	{
		extruder_switch_time += dbCostTime;
	}

    double getCurrentFanSpeed() const 
    {
        if (currentFanSpeed[current_extruder] < 0)
            return 0.0;
        return currentFanSpeed[current_extruder];
    }

    std::string getFileHeaderInfo(const std::vector<bool>& extruder_is_used, const double* print_time, const std::vector<double>& filament_used, const std::vector<double>& filament_weight, const std::vector<std::string>& mat_names, const std::vector<std::string>& mat_keys);

    void updateFileHeaderInfo(const std::vector<bool>& extruder_is_used, const double* print_time, const std::vector<double>& filament_used, const std::vector<double>& filament_weights, const std::vector<std::string>& mat_names, const std::vector<std::string>& mat_keys);

    std::string floatconvertstring(const float ftNumber, const int nStrLen);
};
