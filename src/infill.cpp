...
namespace cura 
{
Polygons Infill::generateWallToolPaths(std::vector<VariableWidthLines>& toolpaths, Polygons& outer_contour, const size_t wall_line_count, const coord_t line_width, const coord_t infill_overlap, const SettingsBaseVirtual& settings)
{
    ...
    if (wall_line_count > 0)
    {
        ...
        wall_toolpaths.pushToolPaths(toolpaths, settings);
        inner_contour = wall_toolpaths.getInnerContour(settings);
    }
    else
    {
...
void Infill::generate(std::vector<VariableWidthLines>& toolpaths, Polygons& result_polygons, Polygons& result_lines, const SettingsBaseVirtual& settings, const SierpinskiFillProvider* cross_fill_provider, const SliceMeshStorage* mesh)
{
    ...
    if (pattern == EFillMethod::ZIG_ZAG
        || (zig_zaggify && (pattern == EFillMethod::LINES
        ...
        || infill_multiplier % 2 == 0)
    {
        ...
        std::vector<VariableWidthLines> gap_fill_paths = wall_toolpaths.getToolPaths(settings);
...
void Infill::_generate(std::vector<VariableWidthLines>& toolpaths, Polygons& result_polygons, Polygons& result_lines, const SettingsBaseVirtual& settings, const SierpinskiFillProvider* cross_fill_provider, const SliceMeshStorage* mesh)
{
...
void Infill::generateGyroidInfill(Polygons& result_polylines, Polygons& result_polygons)
{
    Polygons line_segments;
    GyroidInfill::generateTotalGyroidInfill(line_segments, zig_zaggify, line_distance, inner_contour, z);
    PolylineStitcher<Polygons, Polygon, Point>::stitch(line_segments, result_polylines, result_polygons, infill_line_width);
}

void Infill::generateConcentricInfill(std::vector<VariableWidthLines>& toolpaths, const SettingsBaseVirtual& settings)
{
    ...
    while (true)
    {
        ...
        const std::vector<VariableWidthLines> inset_paths = wall_toolpaths.getToolPaths(settings);
        toolpaths.insert(toolpaths.end(), inset_paths.begin(), inset_paths.end());

        current_inset = wall_toolpaths.getInnerContour(settings);
    }
}
...
void Infill::addLineInfill(Polygons& result, const PointMatrix& rotation_matrix, const int scanline_min_idx, const int line_distance, const AABB boundary, std::vector<std::vector<int64_t>>& cut_list, int64_t shift)
{
    ...
    for(int64_t x = scanline_min_idx * line_distance + shift; x < boundary.max.X; x += line_distance)
    {
        ...
        {
            if (crossings[crossing_idx + 1] - crossings[crossing_idx] < infill_line_width / 10)
			...
			Point p0 = rotation_matrix.unapply(Point(x, crossings[crossing_idx]));
			Point p1 = rotation_matrix.unapply(Point(x, crossings[crossing_idx + 1]));
        }
        scanline_idx += 1;
    }
}
