...
class Infill 
{
    static constexpr int perimeter_gaps_extra_offset = 0; // extra offset so that the perimeter gaps aren't created everywhere due to rounding errors
...
public:
     ...
     */
    void generate(std::vector<VariableWidthLines>& toolpaths, Polygons& result_polygons, Polygons& result_lines, const SettingsBaseVirtual& settings, const SierpinskiFillProvider* cross_fill_provider = nullptr, const SliceMeshStorage* mesh = nullptr);
     ...
     */
    static Polygons generateWallToolPaths(std::vector<VariableWidthLines>& toolpaths, Polygons& outer_contour, const size_t wall_line_count, const coord_t line_width, const coord_t infill_overlap, const SettingsBaseVirtual& settings);

private:
    /*!
     * Generate the infill pattern without the infill_multiplier functionality
     */
    void _generate(std::vector<VariableWidthLines>& toolpaths, Polygons& result_polygons, Polygons& result_lines, const SettingsBaseVirtual& settings, const SierpinskiFillProvider* cross_fill_provider = nullptr, const SliceMeshStorage* mesh = nullptr);
     ...
     */
    void generateConcentricInfill(std::vector<VariableWidthLines>& toolpaths, const SettingsBaseVirtual& settings);
