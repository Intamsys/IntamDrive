...
std::list<SierpinskiFill::SierpinskiTriangle*>::iterator SierpinskiFill::subdivide(std::list<SierpinskiTriangle*>::iterator begin, std::list<SierpinskiTriangle*>::iterator end, bool redistribute_errors)
{
    ...
    int first_child_pos = findSierpinskiTriangleListPos(begin) - 1;
    ...
    std::list<SierpinskiFill::SierpinskiTriangle*>::iterator first_child_it = sequence.begin();

    std::advance(first_child_it, first_child_pos + 1);

    int last_child_pos = findSierpinskiTriangleListPos(begin);
    std::list<SierpinskiTriangle*>::iterator last_child_it = sequence.begin();
    std::advance(last_child_it, last_child_pos - 1);
...
int SierpinskiFill::findSierpinskiTriangleListPos(std::list<SierpinskiTriangle*>::iterator itr)
{
    int npos = -1;
    SierpinskiTriangle* tri = *itr;
    std::list<SierpinskiTriangle*>::iterator itf = std::find(sequence.begin(), sequence.end(), tri);
    if (itf != sequence.end())
    {
        npos = std::distance(sequence.begin(), itf);
    }
    return npos; 
}
