...
void createLayerWithParts(SliceMeshStorage& mesh, SliceLayer& storageLayer, SlicerLayer* layer, unsigned int layer_nr, bool union_layers, bool union_all_remove_holes)
{
	...
	const coord_t xy_contour_offset = mesh.getSettingInMicrons("xy_contour_compensation_size");
	const coord_t xy_hole_offset = mesh.getSettingInMicrons("xy_hole_compensation_size");

    const int expand_line_count  = mesh.getSettingAsCount("expand_line_count");
    const int expand_layer_count = mesh.getSettingAsCount("expand_layer_count");

    const coord_t line_width = mesh.getSettingInMicrons("wall_line_width_0");

    const int expandline_airgap = mesh.getSettingInMicrons("expand_line_air_gap");

    for(unsigned int i=0; i<result.size(); i++)
    {
        storageLayer.parts.emplace_back();
		if (xy_contour_offset != 0 || xy_hole_offset != 0)
		{
			...
			for (const PolygonRef poly : result[i])
			{
				if (poly.orientation())
				{
					if(xy_contour_offset != 0)
						outline.add(poly.offset(xy_contour_offset));
					else
						outline.add(poly);
				}
				else
				{
					if (xy_hole_offset != 0)
						holes.add(poly.offset(xy_hole_offset));
					else
						holes.add(poly);
				}
			}
...
#pragma omp parallel for default(none) shared(mesh,slicer) firstprivate(union_layers,union_all_remove_holes) schedule(dynamic)
    for (unsigned int layer_nr = 0; layer_nr < total_layers; layer_nr++)
    {
        ...
        createLayerWithParts(mesh, layer_storage, &slice_layer, layer_nr, union_layers, union_all_remove_holes);
    }
