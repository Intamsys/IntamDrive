...
public:
    ...
    Point start_point; //!< A location near the prefered start location
    const ZSeamConfig seam_config;

    PathOrderOptimizer(const Point start_point, const ZSeamConfig seam_config = ZSeamConfig(), const bool detect_loops = false, const Polygons* combing_boundary = nullptr, const bool reverse_direction = false, const std::set<std::pair<PathType, PathType>>& order_requirements = no_order_requirements)
...
protected:
    ...
    static const std::set<std::pair<PathType, PathType>> no_order_requirements;

    /*!
     * Order requirements on the paths.
     * For each pair the first needs to be printe before the second.
     */
    const std::set<std::pair<PathType, PathType>>* order_requirements;
...
template<typename PathType>
const std::set<std::pair<PathType, PathType>> PathOrderOptimizer<PathType>::no_order_requirements;
