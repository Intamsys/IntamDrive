...
coord_t SkinInfillAreaComputation::getTopSkinLineWidth(const SliceDataStorage& storage, const SliceMeshStorage& mesh, int layer_nr)
{
	coord_t top_skin_line_width = mesh.getSettingInMicrons("top_skin_line_width");
	if (layer_nr == 0)
	{
		const ExtruderTrain& train_skin = *storage.meshgroup->getExtruderTrain(mesh.getSettingAsExtruderNr("top_bottom_extruder_nr"));
		top_skin_line_width *= train_skin.getSettingAsRatio("initial_layer_line_width_factor");
	}
	return top_skin_line_width;
}

coord_t SkinInfillAreaComputation::getBottomSkinLineWidth(const SliceDataStorage& storage, const SliceMeshStorage& mesh, int layer_nr)
{
	coord_t bottom_skin_line_width = mesh.getSettingInMicrons("bottom_skin_line_width");
	if (layer_nr == 0)
	{
		const ExtruderTrain& train_skin = *storage.meshgroup->getExtruderTrain(mesh.getSettingAsExtruderNr("top_bottom_extruder_nr"));
		bottom_skin_line_width *= train_skin.getSettingAsRatio("initial_layer_line_width_factor");
	}
	return bottom_skin_line_width;
}
...
, initial_bottom_layer_count(mesh.getSettingAsCount("initial_bottom_layers"))
, top_layer_count(mesh.getSettingAsCount("top_layers"))
, wall_line_count(mesh.getSettingAsCount("wall_line_count"))
, top_skin_line_width(getTopSkinLineWidth(storage, mesh, layer_nr))
, bottom_skin_line_width(getBottomSkinLineWidth(storage, mesh, layer_nr))
...
void SkinInfillAreaComputation::generateSkinsAndInfill()
{
    ...
    for (unsigned int part_nr = 0; part_nr < layer->parts.size(); part_nr++)
    {
        ...
        generateFlooring(part);
    }
}

void SkinInfillAreaComputation::generateSinglePartSkinsAndInfill(SliceLayerPart& part)
{
    ...
    {
        int infill_wall_count = mesh.getSettingAsCount("wall_line_count") - 1;
        if(infill_wall_count>0)
            part.setTopAndBottomLayerInfillWallCount(infill_wall_count);
    }
}
...
void SkinInfillAreaComputation::generateSkinAndInfillAreas(SliceLayerPart& part)
{
	...
	double min_area = std::min(mesh.getSettingInMillimeters("min_skin_area"), MIN_AREA_SIZE);
    skin.removeSmallAreas(min_area);
    ...
    for (PolygonsPart& skin_area_part : skin.splitIntoParts())
    {
		...
		Polygons intersection_down = skin_area_part.intersection(bottom_skin);
        Polygons intersection_upper = skin_area_part.intersection(top_skin);

        if (intersection_upper.size() > 0)
        {
            part.skin_parts.back().skin_position_type = SkinPositionType::TOP;
        }
        else
        {
            part.skin_parts.back().skin_position_type = SkinPositionType::BOTTOM;
        }
    }
}
...
void SkinInfillAreaComputation::calculateBottomSkin(const SliceLayerPart& part, Polygons& downskin)
{
    ...
    if (static_cast<int>(layer_nr - bottom_layer_count) < 0)
    {
        return;
    }

    LayerIndex bottom_check_start_layer_idx = std::max(int(0), layer_nr - bottom_layer_count); //从哪一层开始检查
    Polygons not_air = getOutlineOnLayer(part, bottom_check_start_layer_idx);//得到这一层的轮廓线
    ...
    const double min_infill_area = mesh.getSettingInMillimeters("min_infill_area");
...
{
    for (int inset_idx = 0; inset_idx < skin_inset_count; inset_idx++) //skin_inset_count:Extra Skin Wall Count
    {
        ...
        if (inset_idx == 0)
        {
			...
			if (skin_part.skin_position_type == SkinPositionType::BOTTOM)
				skin_part.insets[0] = skin_part.outline.offset(-bottom_skin_line_width / 2 + 10);
			else
				skin_part.insets[0] = skin_part.outline.offset(-top_skin_line_width / 2 + 10);
        }
        else
        {
			if (skin_part.skin_position_type == SkinPositionType::BOTTOM)
				skin_part.insets[inset_idx] = skin_part.insets[inset_idx - 1].offset(-bottom_skin_line_width);
			else
				skin_part.insets[inset_idx] = skin_part.insets[inset_idx - 1].offset(-top_skin_line_width);
        }
...
{  
	...
	if (skin_part.skin_position_type == SkinPositionType::BOTTOM)
		skin_part.inner_infill = innermost_inset.offset(-bottom_skin_line_width / 2);
	else
		skin_part.inner_infill = innermost_inset.offset(-top_skin_line_width / 2);
}
...
void SkinInfillAreaComputation::generateRoofing(SliceLayerPart& part)
{
    ...
    for (SkinPart& skin_part : part.skin_parts)
    {
        ...
        if (!skin_part.roofing_fill.empty()
            && layer_nr > 0
            && mesh.getSettingAsFillMethod("roofing_pattern") != EFillMethod::CONCENTRIC
            && mesh.getSettingAsFillMethod("top_bottom_pattern") == EFillMethod::CONCENTRIC)
        {
            Polygons filled_area_above = generateFilledAreaAbove(part, roofing_layer_count);
            skin_part.roofing_fill = skin_part.outline.difference(filled_area_above);
            skin_part.skin_fill = skin_part.outline.intersection(filled_area_above);
        ...
        if (!skin_part.roofing_fill.empty()
            && !skin_part.skin_fill.empty()
            && layer_nr > 0
            && mesh.getSettingAsFillMethod("roofing_pattern") == mesh.getSettingAsFillMethod("top_bottom_pattern"))
        {
            skin_part.roofing_fill = skin_part.roofing_fill.unionPolygons(skin_part.skin_fill);
            skin_part.skin_fill = skin_part.skin_fill.difference(skin_part.roofing_fill);
        }
    }
}

void SkinInfillAreaComputation::generateFlooring(SliceLayerPart& part)
{
    ...
    for (SkinPart& skin_part : part.skin_parts)
    {
        ...
        Polygons filled_area_below = generateFilledAreaBelow(part, flooring_layer_count);
        skin_part.flooring_fill = skin_part.outline.difference(filled_area_below);
        skin_part.skin_fill = skin_part.outline.intersection(filled_area_below);
        // Insets are NOT generated for any layer if the top/bottom pattern is concentric.
        // In this case, we still want to generate insets for the roofing layers based on the extra skin wall count,
        // if the roofing pattern is not concentric.
            ...
            && mesh.getSettingAsFillMethod("top_bottom_pattern") == EFillMethod::CONCENTRIC)
        {
            ...
            skin_part.skin_fill = skin_part.outline.intersection(filled_area_below);
            const bool concentric_skinfill_pattern =
                mesh.getSettingAsFillMethod("flooring_pattern") == EFillMethod::CONCENTRIC
                && mesh.getSettingAsFillMethod("top_bottom_pattern") != EFillMethod::CONCENTRIC;

            // If the pattern is concentric, ONLY use insets.
            // In this case, we still want to generate skinfill for the flooring layers,
            // but only if the roofing pattern is not concentric.
            if (!skin_part.flooring_fill.empty() && layer_nr >= 0)
            {
                // Regenerate the no_air_below, and recalculate the inner and flooring infills,
                // taking into account the extra skin wall count (only for the flooring layers).
                if (!concentric_skinfill_pattern)
                {
                    regenerateflooringFillAndInnerInfill(part, skin_part);
                }
            }
            // On the contrary, unwanted insets are generated for flooring layers because of the non-concentric top/bottom pattern.
            // In such cases we want to clear the skin insets first and then regenerate the proper flooring fill and inner infill
            // in the concentric flooring_pattern.
            else if (!skin_part.flooring_fill.empty() && skin_part.skin_fill.empty() && layer_nr >= 0 && concentric_skinfill_pattern)
            {
                // Clear the skin insets for the roofing layers and regenerate the roofing fill and inner infill without taking into
                // account the Extra Skin Wall Count.
                skin_part.inset_paths.clear();
        ...
        if (!skin_part.flooring_fill.empty()
            && !skin_part.skin_fill.empty()
            && layer_nr > 0
            && mesh.getSettingAsFillMethod("flooring_pattern") == mesh.getSettingAsFillMethod("top_bottom_pattern"))
        {
            skin_part.flooring_fill = skin_part.flooring_fill.unionPolygons(skin_part.skin_fill);
            skin_part.skin_fill = skin_part.skin_fill.difference(skin_part.flooring_fill);
        }
    }
}
...
void SkinInfillAreaComputation::generateGradualInfill(SliceMeshStorage& mesh, unsigned int gradual_infill_step_height, unsigned int max_infill_steps)
{
    ...
    const int infill_wall_count = mesh.getSettingAsCount("infill_wall_line_count");
    const int infill_wall_width = mesh.getSettingInMicrons("infill_line_width");
    const int infill_overlap = mesh.getSettingInMicrons("infill_overlap_mm");
    ...
    { // loop also over layers which don't contain infill cause of bottom_ and top_layer to initialize their infill_area_per_combine_per_density
        ...
        for (SliceLayerPart& part : layer.parts)
        {
            ...
            const Polygons& infill_area = Infill::generateWallToolPaths(part.infill_wall_toolpaths, part.getOwnInfillArea(),infill_wall_count, infill_wall_width, infill_overlap, mesh);
...
{
    const size_t wall_idx = std::min(size_t(2), size_t(mesh.getSettingAsCount("wall_line_count")));
...
void SkinInfillAreaComputation::regenerateflooringFillAndInnerInfill(SliceLayerPart& part, SkinPart& skin_part)
{
    const size_t flooring_layer_count = std::min(mesh.getSettingAsCount("flooring_layer_count"), mesh.getSettingAsCount("bottom_layers"));

    Polygons no_air_below = generateFilledAreaBelow(part, flooring_layer_count);
    skin_part.flooring_fill = skin_part.outline.difference(no_air_below);
...
void SkinInfillAreaComputation::generateInfillSupport(SliceMeshStorage& mesh)
{
    ...
    const AngleRadians support_angle = mesh.getSettingInAngleRadians("infill_support_angle");
