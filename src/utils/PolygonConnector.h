...
protected:
    ...
    std::optional<PolygonBridge<Polygonal>> findConnection(Polygonal& from_poly, std::vector<Polygonal>& to_polygons)
    {
		...
		std::optional<PolygonBridge<Polygonal>> result = std::optional<PolygonBridge<Polygonal>>();

        if(best_connection)
        {
			result = PolygonBridge<Polygonal>(*best_connection, *best_second_connection);
			return result;
        }
        else
        {
			return std::optional<PolygonBridge<Polygonal>>();
        }
    }
    ...
    std::optional<std::pair<Point, size_t>> walkUntilDistanceFromLine(const Polygonal& poly, const size_t start_index, const coord_t distance, const Point& line_a, const Point& line_b, const short direction)
    {
        ...
        if(line_magnitude == 0)
        {
            return std::optional<std::pair<Point, size_t>>(); //Line doesn't have a direction, so we can't be on any one side of it.
        }
        ...
        for(size_t index = (start_index + direction + poly_size) % poly_size; index != start_index; index = (index + direction + poly_size) % poly_size)
        {
            ...
            if(std::abs(vertex_distance) >= distance) //Further away from the line than the threshold.
            {
				...
				std::optional<std::pair<Point, size_t>> result = std::optional<std::pair<Point, size_t>>();

				result = std::make_pair(interpolated_point, (direction == +1) ? previous_index : index);

				return result;
            }
        }
        return  std::optional<std::pair<Point, size_t>>(); //None of the vertices were far enough away from the line.
    }
    ...
    {
        std::optional<PolygonConnection<Polygonal>> result = std::optional<PolygonConnection<Polygonal>>();
