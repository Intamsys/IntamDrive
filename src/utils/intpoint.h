...
INLINE bool normal(const Point& p0, float& nx, float& ny)
{
    coord_t _len = vSize(p0);
    if (_len < 1)
    {
...
INLINE double radian(const Point& p0, const Point& p1, const Point& p2)
{
    double fx0 = INT2MM(p0.X);
    double fy0 = INT2MM(p0.Y);
    double fx1 = INT2MM(p1.X);
    double fy1 = INT2MM(p1.Y);
    double fx2 = INT2MM(p2.X);
    double fy2 = INT2MM(p2.Y);
    ...
    double de = (fxv0 * fxv1 + fyv0 * fyv1);
    double dt0 = std::sqrt(fxv0 * fxv0 + fyv0 * fyv0);
    double dt1 = std::sqrt(fxv1 * fxv1 + fyv1 * fyv1);

    if (dt0 * dt1 == 0.0f)
        return 0.0f;

    double dv = de / (dt0 * dt1);

    if (dv > 1.0) dv = 1.0;
    if (dv < -1.0) dv = -1.0;
    double dbr = acos(dv);
    //double dba = acos(dv) * 180.0 / M_PI;
    return dbr;
}

INLINE double radian(const Point& p0, const Point& p1)
{
    double fx0 = INT2MM(p0.X);
    double fy0 = INT2MM(p0.Y);
    double fx1 = INT2MM(p1.X);
    ...
    double de = (fx0 * fx1 + fy0 * fy1);
    double dt0 = std::sqrt(fx0 * fx0 + fy0 * fy0);
    double dt1 = std::sqrt(fx1 * fx1 + fy1 * fy1);
...
INLINE double angle(const Point& p0, const Point& p1)
{
    double dbr = radian(p0, p1);
    double a = dbr / M_PI * 180.0;
    return a;
}
